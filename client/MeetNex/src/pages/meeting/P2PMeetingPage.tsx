...
const P2PMeetingPage: React.FC<P2PMeetingPageProps> = ({
  ...
  const peerPreviewRef = useRef<HTMLVideoElement | null>(null);
  ...
  useEffect(() => {
    ...
    if (peerPreviewRef.current && remoteStream) {
      peerPreviewRef.current.srcObject = remoteStream;
    }
  }, [remoteStream]);
      ...
      <AnimatePresence>
        {isSharing && (
          <motion.div
            ...
            exit={{ opacity: 0, x: 20 }}
            className="w-[25%] flex flex-col justify-start pt-4"
          >
            <motion.div
