...
import { Send, Image, FileText, X, Plus, Smile } from "lucide-react";
...
const ChatInput: React.FC<ChatInputProps> = ({ onSend, disabled, placeholder = "Type a message..." }) => {
  ...
  const onEmojiClick = (emojiData: any) => {
    ...
    // Keep focus on textarea after picking emoji
  ...
  return (
    <div className="relative flex flex-col gap-2 w-full font-sans">
      ...
      <div className="group relative flex items-end gap-2 bg-gray-100/50 hover:bg-gray-100 focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-500/20 transition-all duration-300 border-none rounded-[24px] p-1.5 shadow-sm">
        ...
        <div className="relative" ref={optionsRef}>
          ...
          {showOptions && (
            <div className="absolute bottom-12 left-0 w-44 bg-white border border-gray-100 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] p-1.5 z-[60] animate-in slide-in-from-bottom-2 duration-200">
              <button type="button" onClick={() => imageInputRef.current?.click()} className="flex items-center gap-3 w-full px-3 py-2.5 hover:bg-gray-50 rounded-xl text-sm text-gray-700 outline-none">
                <div className="w-8 h-8 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center"><Image size={18} /></div>
                Images
              </button>
              <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-3 w-full px-3 py-2.5 hover:bg-gray-50 rounded-xl text-sm text-gray-700 outline-none">
                <div className="w-8 h-8 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center"><FileText size={18} /></div>
                Documents
              </button>
            </div>
          )}
        ...
        <input ref={imageInputRef} type="file" accept="image/*" multiple hidden onChange={(e) => {
            if(e.target.files) setAttachments(prev => [...prev, ...Array.from(e.target.files!).map(f => ({ file: f, type: 'image' as const }))]);
            setShowOptions(false);
        }} />
        <input ref={fileInputRef} type="file" multiple hidden onChange={(e) => {
            if(e.target.files) setAttachments(prev => [...prev, ...Array.from(e.target.files!).map(f => ({ file: f, type: 'file' as const }))]);
            setShowOptions(false);
        }} />
        ...
        <textarea
          ...
          className="flex-1 bg-transparent border-none focus:ring-0 outline-none text-[15px] text-gray-800 py-2 resize-none max-h-[150px] placeholder:text-gray-400 leading-snug 
          scrollbar-none transition-all duration-200
        ...
        <button
          ...
          className="h-9 w-9 flex items-center justify-center rounded-full bg-blue-600 text-white shadow-lg shadow-blue-500/20 hover:bg-blue-700 hover:scale-105 active:scale-95 disabled:opacity-30 disabled:grayscale disabled:scale-100 transition-all duration-200 outline-none"
        >
